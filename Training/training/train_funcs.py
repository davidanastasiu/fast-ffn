...
def test_pruned(model, test_loader, plot=False,
    ...
    with torch.no_grad():  # Evaulate pruned model performance
        for i, data in enumerate(test_loader):
            ...
            local_batch, local_labels = data
            if(i==0):
                print(" writing local labels \n")
                #print(local_labels)
                #np.savetxt('output/local_labels.txt', local_labels.numpy())
            local_batch, local_labels = local_batch.to(device), local_labels.to(device)
            outputs = model(local_batch.float())
            if(i==0):
                ...
                #np.savetxt('output/outputs.txt', outputs_cpu.numpy())
            _, preds = torch.max(outputs, 1)
            if(i==0):
                print("Writing Preds \n")
                #print(preds)
                #preds_cpu = preds.cpu()
                #np.savetxt('output/preds.txt', preds_cpu.numpy())
            predlist = torch.cat([predlist, preds.view(-1).cpu()])
            if(i==0):
                print("Writing Predlist \n")
                #print(predlist)
                #predlist_cpu = predlist.cpu()
                #np.savetxt('output/predlist.txt', predlist_cpu.numpy())
            lbllist = torch.cat([lbllist, torch.max(local_labels, 1)[1].view(-1).cpu()])
            if(i==0):
                print("writing lbllist \n")
                #print(lbllist)
                #lbllist_cpu = lbllist.cpu()
                #np.savetxt('output/lbllist.txt', lbllist_cpu.numpy())
        outputs = outputs.cpu()
        local_labels = local_labels.cpu()
        predict_test = outputs.numpy()
        accuracy_score_value_list.append(accuracy_score(np.nan_to_num(lbllist.numpy()), np.nan_to_num(predlist.numpy())))
